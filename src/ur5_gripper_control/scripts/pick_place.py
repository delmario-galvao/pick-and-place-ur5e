...
def update_octomap():
    ...
    # for camera in camera_topics:
    #      req = FilterWorkspaceRequest()
    #      req.pointcloud_topic.data = camera + '/points'
    #      req.image_topic.data = camera + '/image_raw'
    #      publish_octomap.call(req)
    # return
...
if __name__ == "__main__":
    try:
        ...
        # update_octomap()
        home_joints = [0.0, -pi/2, pi/2, -pi/2, -pi/2, 0.0]  # sua pose inicial
        arm_group.go(home_joints, wait=True)
        arm_group.stop()
        ...
        # update_octomap()
        open_pos   = [0.0]
        gripper_group.go(open_pos,  wait=True)
        gripper_group.stop()
        ...
        # update_octomap()
        close_pos  = [0.80]
        gripper_group.go(close_pos,  wait=True)
        gripper_group.stop()
